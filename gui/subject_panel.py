...
class SubjectDialog(QDialog):
    ...
    def init_ui(self):
        title = '编辑患者' if self.is_edit else '新增患者'
        ...
        form_layout.addRow('患者 ID *:', self.subject_id_edit)
    ...
    def validate_and_accept(self):
        if not self.subject_id_edit.text().strip():
            QMessageBox.warning(self, '验证错误', '请输入患者 ID')
            return
...
class SubjectPanel(QWidget):
    ...
    def init_ui(self):
        ...
        self.search_edit.setPlaceholderText('搜索患者...')
        ...
        self.table.setHorizontalHeaderLabels(['患者 ID', '姓名', '年龄', '性别', '分组'])
    ...
    def load_subjects(self, subjects=None):
        ...
        for row, subject in enumerate(subjects):
            ...
            self.table.setItem(row, 2, QTableWidgetItem('' if age is None else str(age)))
            self.table.setItem(row, 3, QTableWidgetItem(subject.get('gender', '') or ''))
            self.table.setItem(row, 4, QTableWidgetItem(subject.get('group_type', '') or ''))
    ...
    def add_subject(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if self.db.add_subject(**data):
                ...
                QMessageBox.warning(self, '错误', '患者 ID 已存在')
